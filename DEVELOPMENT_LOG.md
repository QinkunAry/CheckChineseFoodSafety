...
继续打磨公开静态浏览器：让手机端更容易筛选、阅读和点击记录操作按钮，并统一英文风险标签大小写。

### 已完成内容

- 窄屏下收紧 header、main、卡片和记录留白；
- 移动端筛选器保持单列且不再 sticky，避免遮挡内容；
- 输入框和下拉框在移动端使用 16px 字号，减少 iOS 浏览器自动放大；
- 记录操作区改为 flex 布局，让官方链接、复制复核关键词、复制记录 ID 和复制记录链接自动换行；
- 移动端记录操作按钮和 footer 链接改为全宽触控目标；
- `allergen` 风险标签在中文界面显示为“过敏原”，英文界面显示为 “Allergen”；
- 增加静态页面回归测试，确认移动端 CSS、记录操作区和 Allergen 标签存在。

### 决策

保持底层机器标签 `allergen` 不变，只在 UI label 层做展示映射。这样不会影响已有数据、Schema、测试和增量发布逻辑，同时用户界面更自然。

### 下一步

提交并部署后，用手机或浏览器窄屏模式检查筛选器、记录按钮、footer 链接和风险标签显示。下一轮可以做记录反馈入口，或继续做来源升级前的 review 页面。

---

## 2026-09-05 · Round 78 · Source pipeline status and record share helpers

### 本轮目标

解释为什么静态浏览器当前只显示美国、日本、台湾和欧盟，并补上更适合公开页面的记录分享能力。

### 已完成内容

- `summary.json` 新增 `non_published_sources`，列出 `candidate` / `prototype` 等未正式发布来源；
- 静态浏览器新增“正在研究的来源 / Sources in progress”模块，显示加拿大、新西兰、韩国、中国大陆 SAMR、澳新 FSANZ、香港 CFS 等未发布来源状态；
- 明确这些来源已有 probe、candidate 或 prototype 工作，但尚未通过 `implemented` 发布门槛，因此不会混入正式记录；
- 每条记录新增“复制记录 ID / Copy record ID”按钮；
- 每条记录新增“复制记录链接 / Copy record link”按钮，链接使用 `#record-...` 锚点；
- 打开记录链接时，页面会自动用记录 ID 过滤并滚动到对应记录；
- 增加静态页面回归测试，确认未发布来源状态模块和记录分享按钮存在。

### 决策

不把韩国、加拿大、FSANZ、香港 CFS 等候选或原型来源强行显示为正式记录。公开浏览器继续只发布 `implemented` 且通过发布门禁的数据；未发布来源只显示开发状态，避免用户误把候选数据当成正式数据。

### 下一步

提交并部署后，在线上页面检查“正在研究的来源”模块是否出现韩国等来源，并抽查一条记录的复制 ID、复制链接是否可用。下一轮可以继续优化移动端布局，或做记录反馈入口。
...
继续制作 hosted 静态浏览器产品体验：让用户在列表页直接展开单条记录，查看更多可追溯事实字段。

### 已完成内容

- 每条记录卡片新增原生 `<details>` 展开区；
- 展开后显示来源机构、监管地区、原产地字段、记录状态、食品类别、生产者/企业、生产者地点和官方记录号；
- 中英文界面均覆盖详情按钮和字段标签；
- 保持官方事实字段原样展示，不做翻译或改写；
- 增加静态页面回归测试，确认详情展开组件和中英字段标签存在；
- README 路线图记录该产品体验改进。

### 决策

使用 HTML 原生 disclosure 组件，不引入前端框架或复杂状态。详情展开服务于证据追踪，不把记录转化为风险评分或消费建议。

### 下一步

提交并部署后，在线上页面抽查多来源记录的详情展开。下一轮可以做更细的来源/许可说明入口，或为记录列表增加“复制记录链接/ID”的研究便利功能。

---

## 2026-09-03 · Round 73 · Static browser frequent risk combinations

### 本轮目标

继续制作 hosted 静态浏览器产品体验：在页面上提供高频食品类别与风险标签组合摘要，让用户先看到当前数据或当前筛选下的重点分布。

### 已完成内容

- 新增“当前筛选下的高频组合”模块；
- 聚合逻辑基于当前筛选结果动态统计 Top 6 `product_category × hazard_tag`；
- 每个组合显示记录数和涉及来源数；
- 中英文界面均覆盖该摘要模块；
- 明确该摘要只用于提示检索方向，不代表风险概率或消费建议；
- 增加静态页面回归测试，确认聚合模块和中英文文案存在；
- README 路线图记录该产品体验改进。

### 决策

高频组合是 descriptive summary，只描述已发布监管记录中的出现频次。它不是风险评分，不排序“真实危险程度”，也不替代官方原因文本或后续人工复核。

### 下一步

提交并部署后，在线上页面检查默认全量摘要、筛选后摘要变化、中英文切换和空结果状态。下一轮可以继续做来源/许可说明入口，或做更细的记录详情展开体验。

---

## 2026-09-03 · Round 72 · Static browser bilingual UI toggle

### 本轮目标

继续制作 hosted 静态浏览器产品体验：增加中英界面切换，让中文用户更容易理解页面，同时保留英文监管字段和官方原文的证据准确性。

### 已完成内容

- 页面右上角新增语言切换按钮；
- 默认中文界面，切换到英文后会用 `localStorage` 记住用户选择；
- 中英切换覆盖标题说明、阅读边界、来源说明、风险标签说明、措施类型说明、筛选器默认项、统计标签、空结果提示和 official source 链接文字；
- 保持 `product_name`、`reason_summary`、官方记录号和 source URL 等事实字段原样，不做自动翻译；
- 增加静态页面回归测试，确认 bilingual UI 和语言偏好存储逻辑存在；
- README 路线图记录该产品体验改进。

### 决策

本轮只翻译界面层，不翻译监管事实字段。后续如果做官方文本翻译，必须另设可追踪的翻译字段、翻译来源和人工复核状态，不能覆盖原始证据字段。

### 下一步

提交并部署后，在线上页面检查中文默认界面、English 切换、刷新后语言保持，以及筛选器标签是否随语言切换。下一轮可以继续做高风险食品聚合摘要，或做更细的来源/许可说明入口。
