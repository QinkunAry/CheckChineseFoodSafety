...
def _mhlw_reference_from_url(value: Any) -> str | None:
...
def build_japan_release(
    *,
    ...
    if min_records < 1 or max_records < 1 or min_records > max_records:
        raise ValueError("Japan release limits must satisfy 1 <= min_records <= max_records")
    ...
    for record in release_records:
        ...
        url_reference = _mhlw_reference_from_url(record.get("source_url"))
